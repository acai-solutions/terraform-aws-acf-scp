...
def main():
    ...
    if session == None:
        ...
        if expected_org_id != found_org_id or expected_root_ou_id != found_root_ou_id:
            raise (
                Exception(
                    f"Not in the correct AWS Org. Required: {expected_org_id}/{expected_root_ou_id} Found: {found_org_id}/{found_root_ou_id}"
                )
            )
...
def _process_ou_assignments(boto3_client, org_id, root_ou_id, ou_assignments):
    ...
    for path, assignments in ou_assignments.items():
        if path == "/root":
            ...
            ous = _get_ous(
                boto3_client, root_ou_id, path, "/root", f"{org_id}/{root_ou_id}"
            )
            for ou in ous:
